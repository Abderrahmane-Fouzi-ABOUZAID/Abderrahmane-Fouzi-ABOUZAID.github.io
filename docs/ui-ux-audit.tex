...
\midrule
\endhead
\file{index.html} & Hero, profile metadata, actions, CV chooser, project index, experience, education, results, capabilities, contact, footer, and screenshot fallbacks. \\
...
\textbf{Implemented:} ordinary navigation is visible by default. The overlay and toggle appear only after navigation initialization succeeds. All five destinations remain visible with JavaScript disabled. The 404 page deliberately has its home link and recovery actions instead of the five-destination menu.
...
Image declarations & All 15 declarations checked against actual dimensions and nonempty alternatives. \\
...
Axe marks the controls relationship of unopened screenshot dialogs as needing review. The referenced dialog is created before its launcher is enhanced, has the matching unique ID, and opens successfully from all seven launch points. The open-dialog scan has no such unresolved reference. This is a verified hidden-target relationship, not a missing ID.
